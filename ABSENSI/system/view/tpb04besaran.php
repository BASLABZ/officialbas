<div class="navclue abu-abu">
	<div class="clue"><a href="#">Form Setting Besaran TPB04</a></div>
</div>
<form id="periode" method="post"><input type="hidden" id="fbulan" name="bulan"><input type="hidden" id="ftahun" name="tahun"></form>
<div class="canvas-content">
<?php
	$res = $this->db->query("SELECT persen_disiplin, persen_kehadiran, persen_kepatuhan, persen_kinerja, persen_prestasi, persen_inovasi_kreativitas, persen_kemampuan_managerial, persen_kemampuan_interpersonal FROM setting04 LIMIT 0,1");
	list($persen_disiplin, $persen_kehadiran, $persen_kepatuhan, $persen_kinerja, $persen_prestasi, $persen_inovasi_kreativitas, $persen_kemampuan_managerial, $persen_kemampuan_interpersonal) = $this->db->fetchArray($res);	
	
?>		
	<form method="post" onSubmit="return cek(this);">
	<table cellpadding="5" cellspacing="1" class="tabdata">	
		<tr><th class="biru" width="200px"></th><th class="biru">Persentase TPB 04</th></tr>
		<tr>
		...
		<tr>
			<td>Persen Kemampuan Managerial</td>
			<td><input type="text" name="persen_kemampuan_managerial" value="<?php echo $persen_kemampuan_managerial; ?>" size="3" maxlength="2" onkeypress="return keypress(event);"></td>
		</tr>
		<tr>
	...
	function cek(x){
		...
		} else if(trim(x.persen_kemampuan_managerial.value)==""){
			alert('Jumlah persen kemampuan managerial masih kosong.'); x.persen_kemampuan_managerial.focus(); return false;
		...
		} else if (parseInt(x.persen_prestasi.value)+parseInt(x.persen_inovasi_kreativitas.value)+parseInt(x.persen_kemampuan_managerial.value)+parseInt(x.persen_kemampuan_interpersonal.value)!=100) {
			alert('Jumlah persen prestasi, inovasi, managerial dan interpersonal harus 100.'); x.persen_prestasi.focus(); return false;
		} else return true;
