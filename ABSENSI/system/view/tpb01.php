<div class="navclue abu-abu">
	<div class="clue"><a href="#">Form 01 <?php if ($_POST['bulan']) echo 'Triwulan ke '.$this->util->triwulan($_POST['bulan']).' Tahun '.$_POST["tahun"]; ?></a></div>
</div>
<script>
	$(document).ready(function() {
		var kodeskpd = '<?php echo $this->data['id'];?>'; 
		$("#optskpd").change(function(){
			window.location = '?pg=formtpb&mode=tpb01&kodeskpd='+$(this).val();
		});
	...
	function cetak(nip) {
		...
		$("#periode").attr('action','?pg=formtpb&mode=print_tpb01');
		$("#fbulan").val($("#bulan").val());
		$("#ftahun").val($("#tahun").val());
		$("#fnip").val(nip);
		$("#periode").submit();	
	...
	function word(nip) {
		$("#periode").attr('target','frame');
		$("#periode").attr('action','?pg=formtpb&mode=word_tpb01');
		$("#fbulan").val($("#bulan").val());
		$("#ftahun").val($("#tahun").val());
		$("#fnip").val(nip);
		$("#periode").submit();	
